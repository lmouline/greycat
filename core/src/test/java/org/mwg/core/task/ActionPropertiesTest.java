...
public class ActionPropertiesTest {
        ...
        Actions.fromIndexAll("root")
                ...
                .then(new Action() {
                    ...
                    public void eval(TaskContext context) {
                        ...
                        Assert.assertEquals("rel1", result.get(1));
                        Assert.assertEquals("rel2", result.get(2));
                        Assert.assertEquals("rel3", result.get(3));

                        Assert.assertEquals("localIindex1", result.get(4));
                        Assert.assertEquals("localIindex2", result.get(5));
                        Assert.assertEquals("localIindex3", result.get(6));
    ...
    public void testLocalIndex() {
        initGraph();
        Actions.fromIndexAll("root")
                .propertiesWithType(Type.LONG_TO_LONG_ARRAY_MAP)
                .then(new Action() {
                    @Override
                    public void eval(TaskContext context) {
                        TaskResult<String> result = context.result();
