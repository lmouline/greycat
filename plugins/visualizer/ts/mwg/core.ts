              ...
              recursiveAction[0] = (res : org.mwg.task.TaskResult<any>) => {
{
                  ...
                  if (nextResult == null) {
                    ...
                    selfPointer._subTask.executeFromUsing(context, context.wrap(nextResult), org.mwg.plugin.SchedulerAffinity.SAME_THREAD, (result : org.mwg.task.TaskContext) => {
{
              ...
              let nextRes: any = it.next();
              if (nextRes != null) {
                this._subTask.executeFromUsing(context, context.wrap(nextRes), org.mwg.plugin.SchedulerAffinity.SAME_THREAD, (result : org.mwg.task.TaskContext) => {
{
                    result.defineVariable("i", cursor.getAndIncrement());
                  }                }, recursiveAction[0]);
              } else {
            ...
            context.graph().find(context.world(), context.time(), flatIndexName, flatQuery, (result : org.mwg.Node[]) => {
{
        ...
        export class ActionFromIndexAll extends org.mwg.plugin.AbstractTaskAction {
          ...
          public eval(context: org.mwg.task.TaskContext): void {
            ...
            context.graph().findAll(context.world(), context.time(), templatedINdexName, (result : org.mwg.Node[]) => {
{
                context.continueWith(context.wrap(result));
              }            });
          }
            ...
            counter.then(() => {
{
        ...
        export class ActionIndexesNames extends org.mwg.plugin.AbstractTaskAction {
          public eval(context: org.mwg.task.TaskContext): void {
            context.graph().indexes(context.world(), context.time(), (result : string[]) => {
{
                context.continueWith(context.wrap(result));
              }            });
          }
        ...
        export class ActionTraverse extends org.mwg.plugin.AbstractTaskAction {
          ...
          public eval(context: org.mwg.task.TaskContext): void {
            ...
            if (previousResult != null) {
              ...
              let world: number = org.mwg.Constants.NULL_LONG;
              let time: number = org.mwg.Constants.NULL_LONG;
              for (let i: number = 0; i < previousSize; i++) {
                let loop: any = previousResult.get(i);
                if (loop instanceof org.mwg.plugin.AbstractNode) {
                  let casted: org.mwg.Node = <org.mwg.Node>loop;
...
break;
              ...
              let flatName: string = context.template(this._name);
              if (isConsistent) {
                let collected: java.util.ArrayList<number> = new java.util.ArrayList<number>();
                for (let i: number = 0; i < previousSize; i++) {
                  let loop: any = previousResult.get(i);
                  if (loop instanceof org.mwg.plugin.AbstractNode) {
                    let casted: org.mwg.Node = <org.mwg.Node>loop;
                    if (casted.type(flatName) == org.mwg.Type.RELATION) {
                      let flatRel: org.mwg.struct.Relationship = <org.mwg.struct.Relationship>casted.get(flatName);
                      if (flatRel != null) {
                        for (let j: number = 0; j < flatRel.size(); j++) {
                ...
                previousResult.clear();
                let flatCollected: Float64Array = new Float64Array(collected.size());
                for (let i: number = 0; i < collected.size(); i++) {
                  flatCollected[i] = collected.get(i);
                }
                context.graph().lookupAll(world, time, flatCollected, (result : org.mwg.Node[]) => {
{
                    context.continueWith(context.wrap(result));
                  }                });
              } else {
            ...
            recursiveAction[0] = (res : org.mwg.task.TaskResult<any>) => {
{
        ...
        export class CoreTask implements org.mwg.task.Task {
          ...
          public indexesNames(): org.mwg.task.Task {
