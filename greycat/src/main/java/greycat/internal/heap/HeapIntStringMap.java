...
public class HeapIntStringMap implements IntStringMap {
    ...
    HeapIntStringMap(final HeapContainer p_listener) {
    ...
    private void setValue(int i, String newValue) {
        values[i] = newValue;
    }
    ...
    void reallocate(int newCapacity) {
        if (newCapacity > capacity) {
            ...
            keys = new_keys;
            //extend values
            String[] new_values = new String[newCapacity];
            if (values != null) {
    ...
    HeapIntStringMap cloneFor(HeapContainer newParent) {
        HeapIntStringMap cloned = new HeapIntStringMap(newParent);
        ...
        if (values != null) {
            String[] cloned_values = new String[capacity];
    ...
    @Override
    public String get(int requestKey) {
        String result = null;
        synchronized (parent) {
            if (keys != null) {
    ...
    @Override
    public void remove(int requestKey) {
        synchronized (parent) {
    ...
    public final void each(IntStringMapCallBack callback) {
        synchronized (parent) {
            unsafe_each(callback);
        }
    }

    void unsafe_each(IntStringMapCallBack callback) {
        for (int i = 0; i < mapSize; i++) {
            ...
            if (current == Constants.CHUNK_VAL_SEP) {
                if (isFirst) {
                    ...
                    if (!waitingVal) {
                        ...
                        waitingVal = false;
                        put(previousKey, Base64.decodeToStringWithBounds(buffer, previous, cursor));
                    }
                }
                previous = cursor + 1;
            }
